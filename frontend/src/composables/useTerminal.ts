...
export function useTerminal() {
  ...
  // What is parked in the helper textarea between words: the tail of what has
  // already been delivered. A keyboard decides whether a glided word needs a
  // leading space by asking what precedes the cursor, so leaving that there is
  // what makes it produce the space itself — measured on a device, where the
  // second glided word arrives as " wir" rather than "wir" (issue #82).
  let imeContext = "";
  ...
  function takeImeText(): ImeEdit {
    ...
    imeContext = text.slice(-imeContextMax);
  ...
  function deliverIme(edit: ImeEdit) {
    if (!edit.erase && !edit.insert) return;
